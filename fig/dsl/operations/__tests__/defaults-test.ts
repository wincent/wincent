...
describe('equal()', () => {
    ...
    test('handles "array-add"', () => {
        ...
        expect(equal(['a', 'b', 'c'], 'array', ['a', 'c'], 'array-add')).toBe(
            true
        );
    });
...
describe('scanString()', () => {
    ...
    test('it scans a sentence that contains backslashes', () => {
        expect(scanString(new Scanner('"foo \\\\\\\\ bar"'))).toBe(
            'foo \\ bar'
        );
    });
