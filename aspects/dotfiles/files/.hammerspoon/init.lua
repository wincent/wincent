...
-- https://www.benq.com/en-us/monitor/professional/pd2700u/spec.html
local benQPD2700U = '1920x1080' -- real resolution = '3840x2160'

-- https://www.benq.com/en-us/monitor/professional/pd2730s/spec.html
local benQPD2730S = '2560x1440' -- real resolution = '5120x2880'
...
externalDisplay = function()
  return hs.screen.find(benQPD2700U) or hs.screen.find(benQPD2730S) or hs.screen.find(dellU2723QE)
end
...
hs.hotkey.bind({ 'ctrl', 'alt', 'cmd' }, 'escape', 'One-monitor layout (^⌥⌘-F1)', function()
  activateLayout(1)
end)

hs.hotkey.bind({ 'ctrl', 'alt', 'cmd' }, 'f1', 'One-monitor layout (^⌥⌘-F1)', function()
  activateLayout(1)
end)

hs.hotkey.bind({ 'ctrl', 'alt', 'cmd' }, 'f2', 'Two-monitor layout (^⌥⌘-F2)', function()
  activateLayout(2)
end)

hs.hotkey.bind({ 'ctrl', 'alt', 'cmd' }, 'f3', 'Forcing vertical display arrangement ^⌥⌘-F3', function()
  arrangeDisplays('vertical')
end)

hs.hotkey.bind({ 'ctrl', 'alt', 'cmd' }, 'f4', 'Forcing horizontal display arrangement (^⌥⌘-F4)', function()
  arrangeDisplays('horizontal')
end)

hs.hotkey.bind({ 'ctrl', 'alt', 'cmd' }, 'f5', 'Toggling console (^⌥⌘-F5)', function()
  hs.toggleConsole()
end)

hs.hotkey.bind({ 'ctrl', 'alt', 'cmd' }, 'f6', 'Reload (⌃⌥⌘-F6)', function()
  hs.notify.show('Hammerspoon', 'Reloading in the background', 'This may take a few seconds...')
