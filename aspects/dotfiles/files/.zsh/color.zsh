...
EOF
      ...
      # Write jj color overrides based on light/dark scheme.
      local JJ_CONF_D=~/.config/jj/conf.d
      if [ "$BACKGROUND" = "light" ]; then
        cat <<EOF > "$JJ_CONF_D/colors.toml"
[colors]
# Good colors for light schemes.
"diff removed token" = { bg = "#ffdddd" }
"diff added token" = { bg = "#ddffdd" }
EOF
      else
        cat <<EOF > "$JJ_CONF_D/colors.toml"
[colors]
# Good colors for dark schemes.
