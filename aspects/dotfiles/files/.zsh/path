...
function append_to() {
  ...
  if [[ -d "$LOCATION" ]]; then
    ...
    CURRENT=${CURRENT//":$LOCATION:"/:} # Delete (potentially multiple) from middle.
    CURRENT=${CURRENT/#"$LOCATION:"/} # Delete from start.
    CURRENT=${CURRENT/%":$LOCATION"/} # Delete from end.
    typeset -g "$TARGET=${CURRENT:+$CURRENT:}$LOCATION" # Actually append (or if empty, just set).
  fi
}
...
function prepend_to() {
  ...
  if [[ -d "$LOCATION" ]]; then
    ...
    CURRENT=${CURRENT/#"$LOCATION:"/} # Delete from start.
    CURRENT=${CURRENT/%":$LOCATION"/} # Delete from end.
    typeset -g "$TARGET=$LOCATION${CURRENT:+:$CURRENT}" # Actually prepend (or if empty, just set).
  fi
}
...
prepend_to PATH "$HOME/.dprint/bin"
prepend_to PATH "$HOME/.claude/local/bin"
