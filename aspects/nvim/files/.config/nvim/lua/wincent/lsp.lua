...
lsp.init = function()
  ...
  if has_lspconfig then
    ...
    --
    local uses_pnp = function(bufnr)
      return vim.fs.root(bufnr, { '.pnp.cjs', '.pnp.loader.mjs' }) ~= nil
    end

    -- For Yarn PnP projects, point `typescript-language-server` at the
    -- repo-local PnP-aware tsserver shim (generated by `yarn dlx @yarnpkg/sdks
    -- vim`). Without this, the globally installed `tsserver` can't resolve
    -- modules through PnP and exits with code 1.
    --
    -- (We can't use the SDK's `typescript-language-server` shim itself: it is
    -- emitted as `.mjs` but contains CommonJS `require()` calls, which Node
    -- rejects under strict ESM. Only the tsserver shim is needed anyway; the
    -- TLS wrapper does not need to be PnP-aware.)
    local find_pnp_tsserver = function(start)
      local matches = vim.fs.find('.yarn/sdks/typescript/lib/tsserver.js', {
        upward = true,
    ...
    vim.lsp.config('ts_ls', {
      init_options = {
        tsserver = { path = find_pnp_tsserver() },
      },
      root_dir = function(bufnr, on_dir)
        if uses_pnp(bufnr) and original_ts_ls_root_dir then
          original_ts_ls_root_dir(bufnr, on_dir)
        end
      end,
    })
    vim.lsp.enable('ts_ls', vim.fn.executable('typescript-language-server') == 1)

    local original_tsc_root_dir = vim.lsp.config['tsc'].root_dir
    vim.lsp.config('tsc', {
      root_dir = function(bufnr, on_dir)
        if not uses_pnp(bufnr) and original_tsc_root_dir then
          original_tsc_root_dir(bufnr, on_dir)
        end
      end,
    })
    vim.lsp.enable('tsc', vim.fn.executable('tsc') == 1)
